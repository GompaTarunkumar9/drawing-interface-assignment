...
type Tool = 'brush' | 'rectangle' | 'circle' | 'eraser' | 'line' | 'text';
...
export default function DrawingApp() {
  ...
  const draw = ({ nativeEvent }: any) => {
    ...
    } else if (startPos && (selectedTool === 'rectangle' || selectedTool === 'circle' || selectedTool === 'line')) {
  ...
  const endDrawing = () => {
    ...
    if (selectedTool === 'text' && textPosition && textInput) {
      ctxRef.current.font = `${brushSize}px Arial`;
      ...
      <div className="flex flex-col md:flex-row justify-center gap-4 mb-4">
        <div className="flex flex-wrap gap-2 items-center bg-white p-3 rounded-lg shadow">
          ...
          <button
            ...
            className={`p-2 rounded ${selectedTool === 'line' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            📏 Line
          </button>
          <button
            onClick={() => setSelectedTool('text')}
            className={`p-2 rounded ${selectedTool === 'text' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            📝 Text
          </button>
        </div>
